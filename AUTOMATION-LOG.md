...
| Date | Item | Event |
|---|---|---|
| 2026-07-25 | **Ops / resilience** | **Live health re-verification of the Titan pipeline + hardening.** 93/93 tests; webhook/CRM/datastore healthy; dead_letter=0; reconcile dry-run scanned=0 (no leads lost). **Found:** watch channel 1001 sat ~3.5h inside its (then 6h) renewal window overnight UN-renewed — would have lapsed silently, killing live delivery. Root mechanism not fully pinned (Catalyst docs: cron daemon "runs always"); the cron DOES fire autonomously (proven: heartbeat advanced with no forced run) and forced runs execute; renewal code + scopes proven-good (local `maintainWatches` renewed the channel). **Shipped + deployed to Dev:** (1) reconcile **liveness heartbeat** (`reconcile:heartbeat` written each run in `reconcile-core.mjs`, surfaced in `/verify-scheduling` → `reconcile_heartbeat.age_seconds`) — the silent-failure detector this incident showed was missing; (2) **renewal window widened** `renewal_hours` 6→12 (window now spans daytime, cannot fall entirely overnight); +3 tests. **Prod-promotion gotcha logged (IF-7):** the reconcile cron is a *dynamic* SDK cron which Catalyst **cannot migrate to Production** — promotion must use a *pre-defined console cron* + console-created Prod Data Store tables + `titanpool`. Recommend promoting to Production before real lead traffic goes live. |
| 2026-07-23 | **Execution** | **AM0.4 automation COMPLETE — 6 PASS/1 PARTIAL/7 MANUAL/0 FAIL** (`verify-crm.mjs`). Re-probed every "console-only" assumption: **6 were wrong**. Automated via API: module rename→Student Cases, **11-stage pipeline** w/ probabilities+forecast categories, Email duplicate-check, `Assigned Counselor` userlookup, Lost Reason field, 3 roles, assignment rule, sample-data purge; data sharing verified already private. Built reusable `provision-pipeline.mjs` (atomic/idempotent), `verify-crm.mjs` (API acceptance evidence), `validate-crm-schema.mjs` (CI gate), + oauth file-cache & single-flight refresh. 28/28 tests. **Proven-impossible (documented w/ evidence):** validation rule (Zoho API HTTP 500, v7+v8), 5 workflows (action entities read-only), users/2FA (scope + Admin-Panel-only), Cliq (scopes). **Discovered contract:** picklist PATCH is an atomic full-set replace — partial lists de-associate omitted options (hit + repaired mid-session). |
| 2026-07-23 | Execution | **OAuth completed + LIVE provisioning executed.** Old refresh token proven dead (differential probes: client_id/secret/DC all validated; bogus-vs-real token indistinguishable to Zoho). Fresh token minted (founder consent; 1st grant code expired in transit ~1–2 min, 2nd exchanged in seconds). `verify.mjs` ✓ (scopes modules.ALL+settings.ALL, IN DC, 3600s). **`provision-crm.mjs --commit` against production CRM: 19/19 API-addressable fields created, 0 failed, read-back verified; idempotent re-run → 19 skipped.** AM0.4 API-addressable part **DONE**. Remaining: console-only items (rename, pipeline+Lost Reason, dedupe, Assignment Rule, 5 workflows, data sharing) + `Assigned Counselor` user-lookup + evidence A1–A13. |
| 2026-07-22 | Execution | Hardened + fully verified the provisioning pipeline w/o a token: added rollback (deleteField + --rollback), auto-retry (retryAsync, transient-only), testable plan/execute core (injectable API), testable auth-code exchange (exchangeAuthCode). 22/22 tests pass. Mock end-to-end vs the REAL 18-field schema proved: dry-run → commit(19 created, 1 transient retried & recovered) → idempotent re-run(19 skipped) → rollback(19 deleted). auth-url verified (India DC). **Only blocker = OAuth token (founder consent).** |
...
| IF-7 | The reconcile cron is a **dynamic** (SDK-created) Catalyst cron; Catalyst **cannot migrate dynamic crons to Production** (docs). Also surfaced: a silent channel-renewal gap overnight, and the renewal window (6h) could fall entirely within a quiet stretch | Ops/deployment, **no architecture change** — the event-driven design (ADR-006) is unchanged | (a) Prod promotion runbook must create a **pre-defined console cron** in the Prod env (+ console Prod Data Store tables + `titanpool`); (b) added a reconcile **heartbeat** (silent-failure detector); (c) widened `renewal_hours` 6→12. All shipped 2026-07-25. |
